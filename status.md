...
### P7 — Bug 修复：扫描进度条不准确
- [x] **进度条"一秒到 100%"**：扫描实际耗时十分钟，进度条瞬间完成
  - 根因：7 个 Scanner 并行报告局部进度，轻量 Scanner 瞬间到 100%，前端"只增不减"机制锁死
  - 修复方案：ScannerRegistry 计算全局加权进度（fs 50% + browser 15% + system 15% + 其他各 5%），通过 `global_percent` 字段推送到前端
  - 修改范围：`ScanProgress` 结构、`ScannerRegistry::scan_impl`、7 个 scanner 实现、`ProgressEvent`、前端 `ScanPage.tsx`
  - 测试结果：`cargo test --lib` 129 测全绿，`npm run test:run` 51 测全绿
